...
class UpdateJobRequest extends Component {
    ...
    handleSubmit = (event) => {
        ...
        const id = this.props.match.params.jobrequestid;

        const newJobRequestData = { ...this.state.formData }

        //use fetch to make a PUT request with the Data from state that has been populated from
        //the data in the form
        fetch(`${process.env.REACT_APP_API_URL}/api/jobrequests/${id}`, {
            method: "PUT", //make sure whe set our method to PUT when updating records
            headers: {
                'content-type': 'application/json',//make sure we set the content-type headers so the API knows it is recieveing JSON data
                ...generateAuthHeader()
            },
            body: JSON.stringify(newJobRequestData) //send our data form state int he body of the request
        })
            .then((response) => response.json()) // on success, turn the respons into JSON so we can work with it
            .then((data) => {
                const message = "Job Request updated successfully!"
